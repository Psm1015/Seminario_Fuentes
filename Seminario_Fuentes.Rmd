...
## **2. Datos utilizados**
En este apartado es donde se indican las URLs que se van a utilizar durante el desarrollo de este proyecto. 
...
## **8. Análisis de datos**
En el archivo desempleo trabajaremos solo con los datos obtenidos del año 2021 en función del tiempo de búsqueda de empleo.
...
#### **8.1.1 Conversión a DATAFRAME y eliminación de caracteres incorrectos**
...
#### **8.1.2 Eliminación de datos que no necesitamos**
...
datatable(datos_IMC_df)
```


#### **8.1.3 Eliminación de niveles**
...
#### **8.1.4 Creación de datos de obesidad**
...
datatable(datos_obesidad)
```

#### **8.1.5 Datos necesarios para la gráfica de diferentes pesos**
...
#### **8.1.6 Datos necesarios para la gráfica de obesidad**
...
#### **8.2.4 Filtrados de Frecuencia y de Alimentos**
...
Ahora, solo se quiere visualizar el porcentaje de personas que consume pescado durante tres o más días a la semana pero no a diario. Como se ha explicado antes, esto se puede realizar con la función **filter()**
```{r, eval = TRUE, warning = FALSE}
...
Después de esto, se quiere visualizar el porcentaje de personas que consume carne durante tres o más días a la semana pero no a diario.
```{r, eval = TRUE, warning = FALSE}
...
Por último, se quiere observar el porcentaje de personas que consumen comida rápida una o dos veces por semana:
```{r, eval = TRUE, warning = FALSE}
...
#### **8.2.5 Relación de los alimentos con el IMC**
Como el tipo de alimentos que se consumen el Índice de Masa Corporal se encuentran en diferentes set de datos, es necesario unir los datos de un set con los del otro. Esto se puede realizar gracias a la función **join()**, en este caso un **left_join()**. Esta unión queda de la siguiente forma:
```{r, eval = TRUE, warning = FALSE}
...
Como solo se quieren ver las filas en las cuáles las personas sean obesas (IMC >=30 kg/m^2) se necesita la acción de un **filter()**. Además, se quieren eliminar *Ambos sexo* y los valores *Total* de todas las columnas
```{r, eval = TRUE, warning = FALSE}
...
### **9.1 IMC, edad, nivel de estudio y género**
En primer lugar se van a mostrar los diferentes gráficos que se han hecho con el set de datos de IMC.

#### **9.1.1 Porcentaje de pesos de la población española**
...
#### **9.1.2 Relación IMC, edad , nivel de estudio y género**
...
Relaciones_obesidad
```
De esta gráfica obtenemos mucha información, destacando lo siguiente:

En relación a la edad el mayor porcentaje de personas se da en la tercera edad, de los 65 años en adelante. Esto seguramente se deba a que con esas edades las personas llevan una vida mucho más sedentaria y tienen muchas más dificultades para realizar ejercicio físico.

En cuanto al nivel de estudios, destaca que en todos los rangos de edad los que mayor obesidad padecen son aquellas que tienen un menor nivel. Esto nos podía indicar que al tener un menor nivel, se vive en peores condiciones y las personas tienden a descuidarse más. Además se observa claramente que a mayor nivel de estudios hay un porcentaje de obesidad mucho menor.

Respecto al sexo se puede observar que en términos generales hay una mayor obesidad en hombres que en mujeres. Hay ciertas excepciones, como las personas con un nivel de estudio bajo entre los 18 y los 24 años y más de 65 años, ya que en estos casos hay muchas más mujeres que hombres.
Estos resultados nos podrían indicar que en general lás mujeres tienen un mayor cuidado con respecto a su condición física.

### **9.2. Alimentación **
En esta sección se van a mostrar los diferentes gráficos que se han ido haciendo con el set de datos de Alimentación.

#### **9.2.1 Relación de la edad con el tipo de alimentación**
...
Grafica_AlimentosXDia <- ggplot(data = Alimentacion_ADiario, aes(x = Edad, y = value)) +
  ...
  labs(
    title = "Tipo de consumo por Edad",
    x = "Rango de Edad",
    y = "Porcentaje de Personas"
  )
...
#### **9.2.2 Relación de la alimentación de pescado y carne 3 o más veces a la semana pero no a diario por rango de edad y sexo**
...
#### **9.2.3 Relación de la alimentación de comida rápida una o dos veces por semana, por sexo y rangos de edad**
...
#### **9.2.4 ¿Cómo repercute el tipo de alimentación a la obesidad?**
Para esto, se va a crear un gráfico con los datos que se encuentran en *C.Rapida_Obesidad*. En este gráfico se muestra como afecta el consumo de comida rápida en diferentes edades y por sexo.
...
En este gráfico se puede ver que entre 18 y 24 años no hay un alarmante el porcentaje de personas obesas que puede ser debido al consumo de comida rápida. Sin embargo en los otros dos rango de edad si que el porcentaje es significativo y donde habría que poner más atención.

Además, no se observa una relación significativa de la obesidad con el sexo. Solo siendo observable esta diferencia entre 25 y 64 años donde un 10% más de personas obesas que consumen comida rápida son hombres.

### **9.3 Desempleo**

#### **9.3.1 Comparación de desempleo entre hombres y mujeres de distintos grupos de edad**
...
#### **9.3.2 Relación de imc entre hombres y mujeres según el timepo de desempleo**
...
Una vez se han analizado todos los datos, llegamos a la conclusión de que la obesidad es una enfermedad muy compleja ya que es afectada por muchos factores. Entre estos destaca que la edad, el nivel de estudios y el género tienen una correlación directa, ya que en términos generales la mayor parte de personas que la padecen son de la tercera edad, tienen un nivel de estudios bajo y es ligeramente más frecuente en hombres.

De la misma forma, podemos concluir que el desempleo es uno de los factores que afecta en gran parte a la obesidad, viendo como aumenta considerablemente a medida que se alarga el tiempo de búsqueda de empleo tanto en hombres como en mujeres.

Por último, se puede ver que una buena alimentación es clave para prevenir la obesidad ya que abusar, por ejemplo, de comidas rápidas y refrescos azúcarada provocan un aumento de peso significativo. Por lo tanto, es importante llevar una dieta equilibrada para mantener un peso adecuado y que no aparezca la obesidad.
...
- datos.gob.es. (s. f.). Conjunto de datos - datos.gob.es. https://datos.gob.es/es/catalogo

- README. (s. f.). https://cran.r-project.org/web/packages/pxR/readme/README.html

- RPubs - Tablas en R Markdown. (s. f.). https://rpubs.com/Juve_Campos/tablasRMarkdown